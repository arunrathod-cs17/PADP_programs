...
long determine_count(const char *file_name, const char *key, int
ignore_case){
    int key_index=0, key_len = strlen(key) ;long word_count=0;
    char ch;
...
int main() {
    ...
    for(i=0; i<COUNT; i++)
        counts[i] = 0;
    char* my_files[4] ={ "words.txt", "words2.txt", "words3.txt", "words4.txt"} ;
    sprintf(output, " Size\tT1\tT2\tT4\tT8\n") ;
    for(int iter=0; iter<4; iter++) {
        FILE *fp = fopen(my_files[iter] , "r") ;
        fseek(fp, 0L, SEEK_END) ;
        sprintf(output, "%s%6.2lfKB\t", output, ftell(fp) /1024.0) ;
        fclose(fp) ;
        for(int t=1; t<=8; t*=2) {
